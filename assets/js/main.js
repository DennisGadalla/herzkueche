(() => {
  ...
  function initContactJumps() {
    $$('.links a[href="#kontakt"], #angebot a[href="#kontakt"]').forEach((link) => {
      if (link.hasAttribute("data-event-inquiry")) return;
  ...
  function initPosterDialog() {
    ...
    if (!posterLink || !poster || !dialog || typeof dialog.showModal !== "function") return;

    posterLink.addEventListener("click", (event) => {
      event.preventDefault();
      if (dialogPoster) dialogPoster.src = poster.currentSrc || poster.src;
      dialog.showModal();
    });
